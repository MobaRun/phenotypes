...
print(paste0(Sys.time(), "    Loading familial relationship from psam file"))

## --- CHANGED START ---
# CHANGED: Read PLINK 2 .psam robustly:
# - keep '#' in header, then normalize
...
psamDF <- read.table(
  ...
  check.names = FALSE,      # keep '#FID' as-is so we can normalize it
  fill = TRUE               # be tolerant if some fields are missing
)

# Normalize '#FID' -> 'FID' if present
names(psamDF) <- sub("^#", "", names(psamDF))

# Validate required columns that we actually use downstream
required_cols <- c("FID","IID","PAT","MAT","SEX")
missing_cols <- setdiff(required_cols, names(psamDF))
if (length(missing_cols) > 0) {
  stop(paste0("PSAM missing columns: ", paste(missing_cols, collapse = ", ")))
}

# PHENOTYPE is optional; create a placeholder if absent
if (!"PHENOTYPE" %in% names(psamDF)) {
  psamDF$PHENOTYPE <- NA
}

# Map to the column names used later in the script
psamDF <- psamDF %>%
  dplyr::select(
    family_id          = FID,
    sentrix_id         = IID,
    father_sentrix_id  = PAT,
    mother_sentrix_id  = MAT,
    genetic_sex        = SEX
    # 'pheno' not used later; omit to keep changes minimal
  )
## --- CHANGED END ---
...
if (nrow(psamDF) != length(unique(psamDF$sentrix_id))) {
  stop("Duplicate Sentrix Id in the psam file.")
}

psamDF$mother_sentrix_id[! psamDF$mother_sentrix_id %in% motherIdDF$sentrix_id] <- NA
psamDF$father_sentrix_id[! psamDF$father_sentrix_id %in% fatherIdDF$sentrix_id] <- NA
...
rawPheno <- childIdDF %>% 
  filter(
    sentrix_id %in% psamDF$sentrix_id
  ) %>% 
  select(
...
rawPheno <- rawPheno %>% 
  mutate(
    ...
    weight_mother_14m = case_when(
      weight_mother_14m == "100-104" ~ "102",
      weight_mother_14m == "105-109" ~ "107",
      weight_mother_14m == "110-114" ~ "112",
      weight_mother_14m == "115-119" ~ "117",
      weight_mother_14m == "120-129" ~ "125",
      weight_mother_14m == "130-139" ~ "135",
      weight_mother_14m == "140-149" ~ "145",
      weight_mother_14m == "150-159" ~ "155",
      weight_mother_14m == "160-169" ~ "165",
      weight_mother_14m == "170-179" ~ "175",
      T ~ weight_mother_14m
    ),
