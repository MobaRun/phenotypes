...
#names(psamDF) <- c("family_id", "sentrix_id", "father_sentrix_id", "mother_sentrix_id", "genetic_sex", "pheno")
...
## --- CHANGED START ---
# CHANGED: Read PLINK 2 .psam with header; allow any whitespace; keep "#FID" header.
psamDF <- read.table(
  file = genomics_psam_file_path,
  header = TRUE,
  sep = "",                 # any whitespace
...
# CHANGED: Normalize "#FID" -> "FID" if present
names(psamDF) <- sub("^#", "", names(psamDF))

# CHANGED: Minimal validation of expected PSAM columns
required_cols <- c("FID","IID","PAT","MAT","SEX","PHENOTYPE")
missing_cols <- setdiff(required_cols, names(psamDF))
if (length(missing_cols) > 0) {
  stop(paste0("PSAM missing columns: ", paste(missing_cols, collapse = ", ")))
}

# CHANGED: Map to the column names used later in the script
psamDF <- psamDF %>%
  dplyr::select(
    family_id          = FID,
    sentrix_id         = IID,
